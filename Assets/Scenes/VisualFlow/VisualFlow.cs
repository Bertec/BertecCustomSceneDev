...
public class VisualFlow : MonoBehaviour
{
    ...
    public const string PERIPHERALOPTION = "peripheral";
    ...
    public const string KP_1_ID = "KP_1";
    public const string KP_2_ID = "KP_2";
    ...
    [SceneInfo(Key = "visualflowexample", Name = "Visual Flow Example", Scene = "@VisualFlow")]
    public class VisualFlowSceneInfo : SceneInfo
    {
        public VisualFlowSceneInfo()
        {
            ...
            Features.AddKeypointVisualizer(KP_1_ID, "Keypoint 1",
                 new SceneChoices(new()
                {
                    {KEYPOINTVIZOPTION_STAR, "Yellow Star"},    // you can add more visuals here, and you don't have the use the same yellow star as COP.
            ...
            Features.AddKeypointVisualizer(KP_2_ID, "Keypoint 2",
                  new SceneChoices(new()
                 {
                            {KEYPOINTVIZOPTION_STAR, "Yellow Star"},
                            {KEYPOINTVIZOPTION_BALL, "Blue Sphere"},
            ...
            // Peripheral options
            SceneOption peripheral = AddList(PERIPHERALOPTION, "Peripheral");
    ...
    private float minLeftCursorPos, maxRightCursorPos; // set from the Path_... values

    private KeyPointVisualizerEvents.VisualizerMode visualizerMode = KeyPointVisualizerEvents.VisualizerMode.None;
    ...
    void Awake()
    {
        ...
        // Turn the mocap keypoint visuals off until the user selects something
        SetKeypointVisualizer(KP_1_ID, "");
        SetKeypointVisualizer(KP_2_ID, "");

        if (ObstacleObject!=null)
        {
            var kpBounds = Utils.GetBoundingForGameObject(COP_Container);
            var ooBounds = Utils.GetBoundingForGameObject(ObstacleObject.gameObject);
    ...
    private void SetKeypointVisualizer(string keyid, string visualizerOption)
    {
        ...
        else // for the non-cop, each keypoint entry type is tied to it's keypoint id, much like an option
        {
            if (keyid == KP_1_ID)
            {
                MocapKeypoint_KP1?.SetActive(visualizerOption != "");
                MocapKeypoint_KP1_YellowStar?.SetActive(visualizerOption == KEYPOINTVIZOPTION_STAR);
            }
            else if (keyid == KP_2_ID)
            {
                MocapKeypoint_KP2?.SetActive(visualizerOption != "");
                MocapKeypoint_KP2_BlueSphere?.SetActive(visualizerOption == KEYPOINTVIZOPTION_BALL);
                MocapKeypoint_KP2_YellowStar?.SetActive(visualizerOption == KEYPOINTVIZOPTION_STAR);
    ...
    void KeypointPositionUpdated(string keyid, Vector3 newpos)
    {
        ...
        // For the non-COP visualizers, the position on the screen simply tracks the mocap 3d values. The visuals inside can be offset from the container if needed.
        else if (keyid == KP_1_ID)
        {
            if (MocapKeypoint_KP1 != null)
                MocapKeypoint_KP1.transform.localPosition = newpos;
        }
        else if (keyid == KP_2_ID)
        {
            if (MocapKeypoint_KP2 != null)
                MocapKeypoint_KP2.transform.localPosition = newpos;
        }
    }
